...
# 2. Extracting only the measurements on the mean and sd for each mesaurement
        
        # 2.1 Reading column names
          colNames <- colnames(finaldataset)
          
        # 2.2 Create vector for defining ID, mean, and sd
          mean_and_std <- gepl("activtiyID", colNames) 
...
# 5. Creating a second, independet tidy data set with the avg of each variable for each activity and subject
        
        # 5.1 Making a second tidy data set
        tidySet <- aggregate(. ~subjectID, setWithActivityNames, mean)
        tidySet <- tidySet[order(tidySet$subjectID, tidySet$actitvityID), ]
